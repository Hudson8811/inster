$(document).ready(function () {
	const scrollHandler = () => {
		...
		const scrollStop = () => {
			portfolio.scrollTop = afterLoadScrollTop;
		}

		$("[data-fancybox]").fancybox({
			afterLoad: function () {
				afterLoadScrollTop = portfolio.scrollTop;
				portfolio.addEventListener('scroll', scrollStop);
				portfolio.classList.add('scroll-stopped');
			},

			afterClose: function () {
				setTimeout(() => {
					portfolio.removeEventListener('scroll', scrollStop);
					portfolio.classList.remove('scroll-stopped');
				}, 200);
			},
		});
	};
